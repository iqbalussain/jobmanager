    ...
    queryFn: async () => {
      ...
      const transformedData = data?.map(order => ({
        ...
        salesman: order.salesman && typeof order.salesman === 'object' && 'id' in order.salesman
          ...
          : null,
        // Ensure assignee is properly typed with explicit null check
        assignee: order.assignee && 
          ...
          ? {
              id: (order.assignee as any).id,
              full_name: (order.assignee as any).full_name,
              email: (order.assignee as any).email,
            } as Assignee
          : null,
